...
If you have some throuble with secret_key_base running 'rails s', you can generate a random secret key value:
...
Now visit data, for enter in data you need sing in before. if you don't have user yet click in sing up. 
...
If you have some throuble for enter in /data check that mondodb is running.

Check that you have a working installation of "RabbitMQ":http://www.rabbitmq.com, see below the guide for install RabbitMQ.

If RabbitMQ is well instaled when you run rails s then you can see:
...
When you click the button 'create', after save in the databse the product will be send to Cenit Hub, throgth a Push API /add_product that is a HTTP Post request.
...
After save the new product, then will be sent a internal message using RabbitMQ because a new product was created and is required notified the rest of endpoint(s).
...
After save the new product, then will be sent a internal message using RabbitMQ because a product was updated and is required notified the rest of endpoint(s).
...
<img src="app/assets/images/cenit_demo_integration_8.png" alt="Cenit Demo Integration image 8" />
