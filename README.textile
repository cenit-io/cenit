...
* Only one direction, Store I >> Hub >> Store II
* Only is ready the flow for Product object.

Next Steps:
* Add Order object flow.
* Bidirectional comunication, Store I << >> Hub << >> Store II
...
# Add configuration to Store II and add the webhook for 'Add product'.
...
$ c_two.webhooks << Setup::Webhook.create!({:name => 'Add Product', :path => 'add_product'})
$ c_two.webhooks << Setup::Webhook.create!({:name => 'Update Product', :path => 'update_product'})
$ c_two.save
</pre>
...
# go to http://localhost:3001/admin and enter
