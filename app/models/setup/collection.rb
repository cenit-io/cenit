    ...
    def check_dependencies
      flows.each do |flow|
        ...
        [:custom_data_type, :response_data_type].each do |key|
          unless (data_type = flow.send(key)).nil? || ((lib = data_type.try(:validator).try(:library)) && libraries.detect { |v| v == lib })
            libraries << lib
          end
        end
      end
      ...
      translators.each do |translator|
        [:source_data_type, :target_data_type].each do |key|
          unless (data_type = translator.send(key)).nil? || ((lib = data_type.try(:validator).try(:library)) && libraries.detect { |v| v == lib })
            libraries << lib
          end
        end
      ...
      events.each do |event|
        if event.is_a?(Setup::Observer) && (data_type = event.data_type) && !((lib = data_type.try(:validator).try(:library)) && libraries.detect { |v| v == lib })
          libraries << lib
        end
      end
    end
