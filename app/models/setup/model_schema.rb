...
module Setup
  class ModelSchema
    ...
    def load_model
      (model = validates_and_load_model) ? model : nil
    end
    ...
    def notify(call_sym, model=self.name)
      return unless model
      ModelSchema.model_listeners.each do |listener|
        begin
      ...
      unless model.nil?
        begin
          ...
          report[:affected].each do |model|
            begin
              model_schema = ModelSchema.find_by(:name => model.to_s)
    ...
    def reflect_class(name, parent=nil, do_not_create=nil)
      ...
      unless MONGO_TYPES.include?(c.to_s)
        ...
        # ['AfterSave'].each do |cb|
        #   if cb = (parent_module ? parent_module : Object).const_defined?(cb)
        #     puts c.to_s + ' includes ' + cb.to_s
        #     puts "REVIEW THESE"
        #   end
        # end
