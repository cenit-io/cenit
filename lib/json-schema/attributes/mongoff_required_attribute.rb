module JSON
  class Schema
    class MongoffRequiredAttribute < JSON::Schema::Attribute
      class << self
        def validate(current_schema, data, fragments, processor, validator, options = {})
          ...
          schema['required'].each do |property|
            if (property_schema = defined_properties[property]) && property_schema['referenced'] #TODO && mongoff_model_schema?(property_schema)
