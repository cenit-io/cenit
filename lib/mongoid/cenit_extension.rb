  ...
  module CenitExtension
    ...
    module ClassMethods
      ...
      def for_property(property)
        (relation = try(:reflect_on_association, property)).present? && relation.try(:klass)
      end
      ...
      def other_affected_models
        ...
        reflect_on_all_associations(
          ...
          :has_and_belongs_to_many,
          :belongs_to).each do |relation|
            models << relation.klass unless [:has_and_belongs_to_many, :belongs_to].include?(relation.macro) && relation.inverse_of.blank?
          end
        models.uniq
      end
