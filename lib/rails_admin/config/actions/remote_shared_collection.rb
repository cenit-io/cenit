        ...
        register_instance_option :controller do
          proc do
            if Cenit.host == Cenit.homepage
              flash[:warning] = "By the value of your 'homepage' configuration option it seems that you are already at #{Cenit.host}."
              flash[:error] = "Configure a different URL for your Cenit 'host' configuration option to access remote shared collections."
              ...
              if (response = operation.submit(parameters: parameters, verbose_response: true)[:http_response])
                ...
                if id
                  ...
                  if response.code == 200
                    ...
                    flash[:error] = "Unable to retrieve #{@model_config.label_plural} from #{Cenit.host}"
                    redirect_to dashboard_path
                  end
                end
              else
