  ...
  describe "GET /api/v3/setup/storage.json" do
    ...
    context "Fail retrieve all existing Storages"  do
      it "with missing header request" do
        get '/api/v3/setup/storage.json', as: :json
    ...
    context "successful retrieve all existing Storages"  do
      let(:access_token) do
        app = ::Setup::Application.create!(namespace: 'Test', name: 'Storages')
      ...
      it "with authorized token request"  do
        get '/api/v3/setup/storage.json', headers: access_token
        
        expect(response).to have_http_status(:ok)
        expect(response_data[:count]).to be
        expect(response_data[:current_page]).to eq 1
