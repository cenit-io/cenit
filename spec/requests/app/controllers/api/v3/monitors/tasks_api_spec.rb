...
RSpec.describe Api::V3::ApiController, type: :request do
  ...
  describe "GET /api/v3/setup/task.json" do
    let(:response_data) {JSON.parse(response.body, symbolize_names: true)}

    context "Fail retrieve all existing Tasks  "  do
      it "with missing header request" do
        get '/api/v3/setup/task.json', as: :json
      ...
      it "with unauthorized token request"  do
        get '/api/v3/setup/task.json', headers: {
    ...
    context "successful retrieve all existing Tasks"  do
      let(:access_token) do
        app = ::Setup::Application.create!(namespace: 'Test', name: 'Tasks')
      ...
      it "with authorized token request"  do
        get '/api/v3/setup/task.json', headers: access_token
