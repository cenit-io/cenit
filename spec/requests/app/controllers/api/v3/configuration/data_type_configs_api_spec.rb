...
RSpec.describe Api::V3::ApiController, type: :request do
  ...
  describe "GET /api/v3/setup/data_type_config.json" do
    let(:response_data) {JSON.parse(response.body, symbolize_names: true)}

    context "Fail retrieve all existing Data type configs"  do
      it "with missing header request" do
        get '/api/v3/setup/data_type_config.json', as: :json
      ...
      it "with unauthorized token request"  do
        get '/api/v3/setup/data_type_config.json', headers: {
    ...
    context "successful retrieve all existing Data type configs"  do
      let(:access_token) do
        app = ::Setup::Application.create!(namespace: 'Test', name: 'Data type')
      ...
      it "with authorized token request"  do
        get '/api/v3/setup/data_type_config.json', headers: access_token
