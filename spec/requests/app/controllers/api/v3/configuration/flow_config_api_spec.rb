...
RSpec.describe Api::V3::ApiController, type: :request do
  ...
  describe "GET /api/v3/setup/flow_config.json" do
    let(:response_data) {JSON.parse(response.body, symbolize_names: true)}

    context "Fail retrieve all existing Flow configs"  do
      it "with missing header request" do
        get '/api/v3/setup/flow_config.json', as: :json
      ...
      it "with unauthorized token request"  do
        get '/api/v3/setup/flow_config.json', headers: {
    ...
    context "successful retrieve all existing Flow configs"  do
      let(:access_token) do
        app = ::Setup::Application.create!(namespace: 'Test', name: 'Flow configs')
      ...
      it "with authorized token request"  do
        get '/api/v3/setup/flow_config.json', headers: access_token
